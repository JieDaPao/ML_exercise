/**
  ...
  */
import org.apache.spark.mllib.classification.{LogisticRegressionWithLBFGS, LogisticRegressionWithSGD, SVMWithSGD,NaiveBayes}
...
//      """select * from liushi where Target in (0,1)""".stripMargin),Array("uid"),"Target")
    ...
    println("方差：",rows.computeColumnSummaryStatistics().variance)//每列的方差
...
//    }
    ...
    val correct_lrSGD=data_test.map{
      point => if(model_lrSGD.predict(point.features)==point.label)
        1 else 0
    }.sum()/data_test.count()//精确度：0.6060885608856088
    ...
    val correct_lgR=data_test.map{
      point => if(model_lgR.predict(point.features)==point.label)
        1 else 0
    }.sum()/data_test.count()//精确度：0.6060885608856088
